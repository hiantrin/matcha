...
const Registration = () => {
    ...
    const Pressenter =  (e) => {
		const error = {};
        if (e.key === 'Enter' && e.target.value && all.tags.length < 5 && Compare(e.target.value)) {
			const tags = [...all.tags];
            tags.push(e.target.value);
            setAll({...all, tags:[...tags]})
            e.target.value = "";
        } else if (e.key === 'Enter' && !e.target.value) {
			error.tags = "please type a tag";
		} else if (e.key === 'Enter' && all.tags.length === 5 ) {
            e.target.value = "";
    ...
    const deleteitem = (e) => {
		setAll({...all, tags: all.tags.filter(item => item !== e)})
    }
    ...
    <div className='h-auto lg:h-screen w-screen bg-zinc-100 px-[15%] py-[140px] overflow-y-auto'>
        <div className="h-full w-full flex flex-col lg:flex-row">
            <div className='sm:w-[80%] h-full lg:w-[50%] flex flex-col justify-center items-center lg:items-start order-last lg:order-first '>
                ...
                <div className='max-w-[350px] h-[30px] rounded-xl bg-gray-200 flex  pl-3 pr-4 left-shadow '>
                    <input type="text" name="tags" placeholder='Press Enter to Add tags' className='w-[250px] sm:w-[350px] bg-transparent outline-none placeholder:text-[0.7rem] text-sm pr-4 font-bold '  onKeyPress={e => {e.key === 'Enter' && e.preventDefault() ; e.key === 'Enter' && Pressenter(e)}}></input>
                </div>
