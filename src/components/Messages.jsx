...
const Messages = () => {
	...
	const sendMessage = async () => {
		...
		const res = await getInstance(token).post("/storeConversations/storeMessage", {
			receiver: chat.id,
	...
	useEffect(() => {
		socket.on("msgRec", (data) => {
			if(data && data.sender_id == chat.id)
			{
				setAllMessages(prev => prev.concat({
	...
	<div className={`w-full flex flex-col items-center justify-between space-y-4  + ${trans ? 'Tran' : ''}`} key="map">
		{users.map((element, id) => {
			return(
				<div className='min-w-[350px] p-4 flex bg-white rounded-xl items-center  cursor-pointer hover:bg-indigo-600 hover:text-white transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300' onClick={() => {setChat(element);toChat(element)}} key={id}>
					<img src={element.profile_img} className="border rounded-full mr-5 w-12 h-12"></img>
					<h1 className='text-sm  font-bold'>{element.user_name}</h1>
				</div>
		)})}
	</div>
        ...
        <h1 className='text-4xl font-bold'>Chat.</h1>
			...
			<div className={trans ? 'flex flex-col space-y-4' : "hidden"}>
				<div className='min-w-[350px] p-4 flex bg-white rounded-xl items-center'>
					<FontAwesomeIcon icon={faChevronLeft} size="2x" className='py-1 px-2 text-gray-600 mr-3 cursor-pointer' onClick={toBefore}/>
				...
				<div className='w-[350px] rounded-xl  h-[500px] bg-white p-3 flex flex-col justify-between space-y-3'> 
					{mapMsg}
